...
> 每周自动聚合的计算机视觉（CV）论文 / 模型 / 行业动态档案库。

数据源自 [AI HOT](https://aihot.virxact.com) 中文 AI 资讯聚合平台，经关键词筛选后生成**时间线 + 卡片**式速览页面，并按周滚动存档。
...
页面顶部 `.nav-bar` 提供周次导航，可在「本周 / 上周」之间一键切换。
...
│   └── ……                  # 后续每周一个独立存档页
├── README.md               # 本说明
└── build_archive.py        # 构建与推送脚本（见下方「本地构建」）
```
...
- **频率**：每周一自动化触发（定时任务）；
- **流程**：拉取最新数据 → CV 筛选 → 拆为「本周 / 上周」→ 生成 `index.html` 与 `archive/YYYY-MM-DD.html` → 推送 GitHub Pages；
- **推送方式**：`git clone` 现有仓库 → 覆盖生成文件 → 普通 `commit` + `push`（**不 force**，保留历史周存档）。
...
```bash
# 1. 安装依赖（标准库为主，仅需网络访问 AI HOT API）
...
<p align="center">
  Powered by <a href="https://aihot.virxact.com">AI HOT</a> · Generated automatically every Monday
</p>
